...
class PETDataset(Dataset):
    def __init__(self, data_dir, dataset_type='train', transform=None):
        if dataset_type == 'train':
            self.annotation_file = os.path.join(data_dir, 'pet_clean_label_csv_file.csv')
            self.img_dir = os.path.join(data_dir, '페트병_무색유색_오염없음_이미지')
        # elif dataset_type == 'test':
        #     self.annotation_file = os.path.join(data_dir, 'pet_balance_csv_file_TEST.csv')
...
class PETDataModule(LightningDataModule):
    ...
    def __init__(
        self,
        data_dir: str = "/data2/local_datasets/recycling/clean/",
